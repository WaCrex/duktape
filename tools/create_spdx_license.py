...
def main():
    ...
    duktape_pkgname = f'duktape-{duktape_version}.tar.xz'
    now = datetime.datetime.utcnow()
    now = datetime.datetime(now.year, now.month, now.day, now.hour, now.minute, now.second)
    creation_date = Literal(f'{now.isoformat()}Z', datatype=XSD.dateTime)
    ...
    g.add(
        (
            ...
            Literal(
                f'http://duktape.org/{duktape_pkgname}', datatype=XSD.anyURI
            ),
        )
    )
    ...
    g.add(
        (
            spdx_doc,
            RDFS.comment,
            Literal(f'SPDX license for Duktape {duktape_version}'),
        )
    )
