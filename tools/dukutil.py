...
class BitEncoder:
    ...
    def getBytes(self):
        ...
        for i in xrange(self.getNumBytes()):
            ...
            for j in xrange(8):
                off = i*8 + j
                t = (t << 1) if off >= len(self._bits) else (t << 1) + self._bits[off]
            bytes.append(t)
...
class GenerateC:
    ...
    def emitHeader(self, autogen_by):
        ...
        self.emitLine('/*')
        self.emitLine(f' *  Automatically generated by {autogen_by}, do not edit!')
        self.emitLine(' */')
        self.emitLine('')
    ...
    def emitArray(self, data, tablename, visibility=None, typename='char', size=None, intvalues=False, const=True):
        ...
        if isinstance(data, str):
            tmp = [ord(data[i]) for i in xrange(len(data))]
            data = tmp

        size_spec = '%d' % size if size is not None else ''
        visib_qual = f'{visibility} ' if visibility is not None else ''
        const_qual = 'const ' if const else ''
...
def duk_heap_hashstring_dense(x, hash_seed, big_endian=False, strhash16=False):
    ...
    if len(x) <= DUK__STRHASH_SHORTSTRING:
        ...
        while off < len(x):
            left = len(x) - off
            now = left
            now = min(now, DUK__STRHASH_BLOCKSIZE)
